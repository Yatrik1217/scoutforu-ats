...
- **Supabase free-tier Egress (5 GB/month)** is the tightest limit for two live apps. The billing cycle **resets on the 28th**. With the **spend cap ON** you are never charged — worst case, if egress hits 5 GB before the reset the project goes briefly **read-only / returns 402** until the 28th, then clears on its own. The app was tuned to minimise this: the ATS no longer streams heavy job text on every page load, "Find matches" reads the Talent Bank gzipped, and the CRM caches its `db.json` in memory (keep the CRM process **up** — a cold restart re-downloads the whole blob). **Check after the 28th:** Supabase → Billing → *View usage* → Egress should grow slowly. If it climbs fast again, look for a service re-reading large data in a loop (past culprit: the CRM per-minute cron before it was cached).
