...
// This modal covers LOANS and INSURANCE premiums (both post an expense when
// paid). SIPs / investments are added from the Investments page instead.
export function EmiModal({
  ...
  const [f, setF] = useState<EmiForm>(() => ({
    scope,
    type: emi?.type === "insurance" ? "insurance" : "loan",
    name: emi?.name ?? "",
    lender: emi?.lender ?? "",
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-[16px] font-extrabold">
              {emi ? "Edit" : "Add"} {isLoan ? "loan / EMI" : "insurance premium"}
          ...
          <div className="grid gap-3">
            ...
            <div className="flex gap-2">
              ...
              <TypeButton active={!isLoan} onClick={() => set("type", "insurance")} label="Insurance" hint="recurring premium" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder={isLoan ? "e.g. Car Loan" : "e.g. Eterm Insurance"}
                />
              </div>
              <div>
                <label className={label}>{isLoan ? "Lender" : "Provider"} (optional)</label>
                <input
                  className={field}
                  value={f.lender}
                  onChange={(e) => set("lender", e.target.value)}
                  placeholder={isLoan ? "e.g. HDFC Bank" : "e.g. Bajaj Allianz"}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className={label}>{isLoan ? "Monthly EMI (₹)" : "Premium / month (₹)"}</label>
                <NumberInput
                  className={field}
                  value={f.emiAmount}
          ...
          <div className="mt-1 flex justify-end gap-2">
            ...
            >
              {pending ? "Saving…" : emi ? "Save changes" : isLoan ? "Add loan" : "Add premium"}
            </button>
          </div>
        </DialogContent>
      </Dialog>
  ...
  return (
    <button
      ...
      className={`flex-1 rounded-[10px] border px-3 py-2 text-left transition ${
        active
          ? "border-[#16a34a] bg-[#eafaf0]"
          : "border-[#e3e8f0] bg-white hover:border-[#cdd6e4]"
      }`}
    >
      <div className={`text-[13px] font-bold ${active ? "text-[#128a3e]" : "text-[#42506b]"}`}>{text}</div>
      <div className="text-[11px] font-medium text-[#8a94a6]">{hint}</div>
    </button>
  );
}
